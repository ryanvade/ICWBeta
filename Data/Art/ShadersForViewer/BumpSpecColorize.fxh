...
float4 bump_spec_colorize_ps_main(VS_OUTPUT In): COLOR
{
	...
	float3 specTexel = tex2D(SpecSampler,In.Tex2);
	...
	float3 spec = m_light0Specular*pow(ndoth,16)*specTexel.rgb*normalTexel.a;
...
half4 bump_colorize_ps_main(VS_OUTPUT In) : COLOR
{
	...
	half4 specTexel = tex2D(SpecSampler,In.Tex2);
	...
	half3 specular = specTexel * norm_texel.a;
