    ...
    return {
        ...
        execute = function(self)
            ...
            if not ownerTable[self.owner_string] then
                local planet = StoryUtil.FindFriendlyPlanet(self.owner_string)
                if not planet then
                    return
                end
            ...
            if player.Is_Human() then
                if GalacticNewsFeed then
                    GalacticNewsFeed:post {
                        headline = "TEXT_SINGLE_UNIT_RETREAT_PLANET",
                        var = ownerTable[self.owner_string],
                        color = {r = 255, g = 0, b = 0}
                    }
                else
                    StoryUtil.ShowScreenText(
                        "TEXT_SINGLE_UNIT_RETREAT_PLANET",
                        5,
                        ownerTable[self.owner_string],
                        {r = 255, g = 255, b = 255}
                    )
                end
            end
