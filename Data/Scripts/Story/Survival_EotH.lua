...
--*   @Filename:            Survival_EotH.lua
--*   @Last modified by:    [TR]Pox
--*   @Last modified time:  2017-12-21T13:18:24+01:00
...
enemy_types = {
			{"Lancer_Frigate", "Carrack_Cruiser"}, --Level1
			{"Escort_Carrier", "Strike_Cruiser"}, --Level2
			{"Generic_Acclamator_Assault_Ship", "Venator"}, --Level3
			{"Generic_Star_Destroyer", "Generic_Victory_Destroyer_Two"}, --Level4
			{"Generic_Star_Destroyer_Two", "Generic_Tector"}  --Level5
			}


		boss_types = {
			{"Implacable_Star_Destroyer", "Gorgon"},
{"Chimera_Pellaeon_Grand"},
{"Generic_Praetor", "Thalassa"},
{"Generic_Praetor", "Generic_Praetor"},
{"Generic_Praetor", "Generic_Praetor"}
			}
		...
		start_list = {"Budirech", "Kariek_Shieldship", "Vigilance_Gunship"}
		SpawnList(start_list, item_base_marker, underworld, false, false)
	...
	while true do
		...
		if current_wave_alive then
			...
			if units_alive == 0 then
				...
				underworld.Give_Money(new_money)
				Story_Event("ENABLE_REINFORCEMENTS")
				wave = wave + 1
				GlobalValue.Set("WaveCount", wave)
				remaining_units = Find_All_Objects_Of_Type(pirates)
...
function Spawn_Items()
	...
	if item_spawn_chance >= 75 then
		...
		Register_Prox(my_item, Item_Effect, 200, Find_Player("Underworld"))
		--Game_Message("TEXT_FACTION_REBELS")
